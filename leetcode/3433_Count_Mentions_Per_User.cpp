...
public:
    std::vector<int> countMentions(int numberOfUsers, std::vector<std::vector<std::string>>& events) {
        sort(events.begin(), events.end(), [](const std::vector<std::string>& a, const std::vector<std::string>& b) {
            if (a[1] == b[1]) {
        ...
        std::vector<int> offline(numberOfUsers, 0);
        int all = 0;

        for (int i = 0; i < events.size(); i++) {
            if (events[i][0] == "OFFLINE") {
                offline[stoi(events[i][2])] = stoi(events[i][1]) + 60;
            }
            if (events[i][0] == "MESSAGE") {
                if (events[i][2] == "ALL") {
                    ...
                    for (int j = 0; j < numberOfUsers; j++) {
                        if (offline[j] <= stoi(events[i][1])) mentions[j]++;
                    }
                } else {
                    std::string &s = events[i][2];
                    size_t position = 0;
                    ...
                    while (position < s.size()) {
                        ...
                        size_t next;
                        int val = stoi(s.substr(position), &next);
                        mentions[val]++;
